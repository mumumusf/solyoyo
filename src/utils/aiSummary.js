...
export async function sendSumMessage(tokenInfo, replyToMessageId) {
  ...
  if (!summaryResult) {
    console.log(`无法获取 ${tokenInfo.symbol} 的推文总结`);
    return;
  }
  
  const { search_summary, account_summary } = summaryResult;
  
  let message = `\u{1F49B}${tokenInfo.symbol} 推文分析：\n`;
  
  if (account_summary) {
    const formattedAccountSummary = account_summary
      .replace(/\n\s*\n/g, '\n')  
      .trim();  
    message += `<blockquote>官方账号动态：\n${formattedAccountSummary}</blockquote>\n\n`;
  }
  
  if (search_summary) {
    message += `\u{1F49B}社区讨论总结：\n<blockquote>${search_summary}</blockquote>`;
  }
